...
static void ppu_handlePixel(Ppu* ppu, int x, int y) {
  ...
  if(!ppu->forcedBlank) {
    ...
    bool clipColorsToBlack =
      ppu->clipMode == 3 ||
      (ppu->clipMode == 2 && colorWindowState) ||
      (ppu->clipMode == 1 && !colorWindowState);
    if(clipColorsToBlack) {
      r = 0;
      g = 0;
      b = 0;
    }
    int secondLayer = 5; // backdrop
    bool mathEnabled = !ppu->snes->debugDisableColorMath &&
      mainLayer < 6 && ppu->mathEnabled[mainLayer] && !(
    ...
    if((mathEnabled && ppu->addSubscreen) || ppu->pseudoHires || ppu->mode == 5 || ppu->mode == 6) {
      ...
      if(clipColorsToBlack) {
        ...
        g2 = 0;
        b2 = 0;
        // Treat the clipped subscreen as an actual black source rather than
        // "no subscreen", so color math does not incorrectly fall back to the
        // fixed color register on windowed scanlines.
        if(secondLayer == 5) secondLayer = 0;
      }
    }
  ...
  int row = y - 1;
  int mirrorRow = row + 239;
  if(ppu->frameInterlace) {
    row += ppu->evenFrame ? 0 : 239;
    mirrorRow = -1;
  }
  uint8_t subB = ((b2 << 3) | (b2 >> 2)) * ppu->brightness / 15;
  uint8_t subG = ((g2 << 3) | (g2 >> 2)) * ppu->brightness / 15;
  uint8_t subR = ((r2 << 3) | (r2 >> 2)) * ppu->brightness / 15;
  uint8_t mainB = ((b << 3) | (b >> 2)) * ppu->brightness / 15;
  uint8_t mainG = ((g << 3) | (g >> 2)) * ppu->brightness / 15;
  uint8_t mainR = ((r << 3) | (r >> 2)) * ppu->brightness / 15;

  ppu->pixelBuffer[row * 2048 + x * 8 + 0 + ppu->pixelOutputFormat] = subB;
  ppu->pixelBuffer[row * 2048 + x * 8 + 1 + ppu->pixelOutputFormat] = subG;
  ppu->pixelBuffer[row * 2048 + x * 8 + 2 + ppu->pixelOutputFormat] = subR;
  ppu->pixelBuffer[row * 2048 + x * 8 + 4 + ppu->pixelOutputFormat] = mainB;
  ppu->pixelBuffer[row * 2048 + x * 8 + 5 + ppu->pixelOutputFormat] = mainG;
  ppu->pixelBuffer[row * 2048 + x * 8 + 6 + ppu->pixelOutputFormat] = mainR;

  if(mirrorRow >= 0) {
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 0 + ppu->pixelOutputFormat] = subB;
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 1 + ppu->pixelOutputFormat] = subG;
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 2 + ppu->pixelOutputFormat] = subR;
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 4 + ppu->pixelOutputFormat] = mainB;
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 5 + ppu->pixelOutputFormat] = mainG;
    ppu->pixelBuffer[mirrorRow * 2048 + x * 8 + 6 + ppu->pixelOutputFormat] = mainR;
  }
}
