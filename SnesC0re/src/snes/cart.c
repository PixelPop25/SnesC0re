...
Cart* cart_init(Snes* snes) {
  ...
  cart->ram = NULL;
  cart->ramSize = 0;
  cart->batteryDirty = false;
  return cart;
}

void cart_free(Cart* cart) {
  if(cart == NULL) return;
  if(cart->rom != NULL && cart->ownsRom) free(cart->rom);
  if(cart->ram != NULL && cart->ram != cart->ramStorage) free(cart->ram);
  free(cart);
}
...
static void cart_prepare_load(Cart* cart, int type, int romSize, int ramSize) {
  cart->type = type;
  if(cart->rom != NULL && cart->ownsRom) free(cart->rom);
  if(cart->ram != NULL && cart->ram != cart->ramStorage) free(cart->ram);
  cart->ownsRom = false;
  ...
  if(ramSize > (int)sizeof(cart->ramStorage)) {
    ramSize = (int)sizeof(cart->ramStorage);
  }
  if(ramSize > 0) {
    cart->ram = cart->ramStorage;
...
void cart_load(Cart* cart, int type, uint8_t* rom, int romSize, int ramSize) {
  cart_prepare_load(cart, type, romSize, ramSize);
...
void cart_load_owned(Cart* cart, int type, uint8_t* rom, int romSize, int ramSize) {
  cart_prepare_load(cart, type, romSize, ramSize);
  cart->rom = rom;
  cart->ownsRom = false;
}
...
static void cart_writeLorom(Cart* cart, uint8_t bank, uint16_t adr, uint8_t val) {
  if(((bank >= 0x70 && bank < 0x7e) || bank > 0xf0) && adr < 0x8000 && cart->ramSize > 0) {
    // banks 70-7e and f0-ff, adr 0000-7fff
    uint32_t index = (((bank & 0xf) << 15) | adr) & (cart->ramSize - 1);
    if(cart->ram[index] != val) {
      cart->ram[index] = val;
      cart->batteryDirty = true;
    }
  }
}
...
static void cart_writeHirom(Cart* cart, uint8_t bank, uint16_t adr, uint8_t val) {
  ...
  if(bank < 0x40 && adr >= 0x6000 && adr < 0x8000 && cart->ramSize > 0) {
    // banks 00-3f and 80-bf, adr 6000-7fff
    uint32_t index = (((bank & 0x3f) << 13) | (adr & 0x1fff)) & (cart->ramSize - 1);
    if(cart->ram[index] != val) {
      cart->ram[index] = val;
      cart->batteryDirty = true;
    }
